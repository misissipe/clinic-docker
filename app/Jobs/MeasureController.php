...
class MeasureController extends Controller
{
    public function index(){
        ...
        $view = Measure::where('campus',session('campus'))->get();
...
public function edit(Request $request){
    ...
    $edit = Measure::where('id', $this->aes->decrypt($request->id))
...
public function update(Request $request){
    ...
    $chckItem = Measure::where('id',$this->aes->decrypt($measureId))->where("unit_of_measurement",$measurename)->where('campus',session('campus'))->first();

    if (!empty($measurename) && $chckItem) {
        return response()->json(["Error" => 1, "Message" => "Already Exists!"]);
    } else {
        $update = Measure::where('id', $this->aes->decrypt($measureId))
            ->update([
                'campus' => session('campus'),
                "unit_of_measurement" => $measurename,
 ...
 public function delete(Request $request) {
    ...
    $delete=Measure::where('id', $this->aes->decrypt($request->id))
    ->update([

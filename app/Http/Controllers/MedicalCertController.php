...
class MedicalCertController extends Controller
{
    ...
    public function create(Request $request){ 
      ...
      if ($role === 'Employee') {
          $patientId = $request->hhId;
      } else { 
          $patientId = $request->id;  
      }

      $patientId = $this->aes->decrypt($patientId);

      $check = HealthHistory::where('patientId', $patientId)
      ...
      if (empty($check)){
        ...
        $namePatient =  $name->FirstName . ' ' .  substr($name->MiddleName, 0, 1) . '. ' . ' ' . $name->LastName;
        ...
        if ($role === 'Student') {  
          ...
          ->join('health_history as h', function ($join) {
              $join->on('s.StudentNo', '=', 'h.patientId')
                  ->where('h.campus', session('campus'));
          })
          ...
          $employeeId = $this->aes->decrypt($request->hhId ?? $request->id);

          $name = Employee::where('AgencyNumber', $employeeId)
            ->where('campus', session('campus'))
            ->first();

          $view = DB::table('employee_info as e')
            ->join('health_history as h', 'e.AgencyNumber', '=', 'h.patientId')
            ->where('e.AgencyNumber', $employeeId)
          ...
          return view('pages.medical-certificate-patient-information', ['pageConfigs' => $pageConfigs, 'breadcrumbs' => $breadcrumbs], compact('view', 'encryptedId', 'role', 'request','age','newbday','medicalrecord'));        
        }
      return back();
    }
    ...
    public function viewForApprove(Request $request)
    {
      ...
      return view('pages.medical-preview-certificate', ['pageConfigs' => $pageConfigs, 'breadcrumbs' => $breadcrumbs], compact('doctor','response','request','newencryptedId','role','addYear','today'));
    }
    ...
    public function generateCertificate(Request $request){
      ...
      return view('pages.medical-generated-certificate',['pageConfigs'=>$pageConfigs,'breadcrumbs'=>$breadcrumbs],compact('doctor','newbday','age','addYear','response','request','date'));
  }
    ...
    public function saveCert(Request $request){
      ...
      $user =  DB::table('medicalcert')
        ->insert([
          ...
          'doctorIncharged' =>  $doctorIncharged,
          'license' =>  $license,
   ...
   public function generateMCPDF(Request $request){
     ...
     $schoolyr = $schoolyr_start;
      ...
      return Pdf::loadView('pages.medical-certificate-form',compact('response','addYear','doctor','today'))->stream();
   }
   public function generateMCPDFgen(Request $request)
   {
      ...
      return Pdf::loadView('pages.medical-generated-certificate-form',compact('response','addYear','doctor','today'))->stream();
   }
}

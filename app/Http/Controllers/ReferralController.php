...
class ReferralController extends Controller
{
    ...
    public function referalPatientInfo(Request $request){
      ...
      $patient = DB::table('student_info')
          ->where('StudentNo', $this->aes->decrypt($request->id))
          ->where('campus', session('campus'))
          ->first();

      $ids = $patient->StudentNo;
     

    } elseif ($role === 'Employee') {
        $patient = DB::table('employee_info')
            ->where('id', $this->aes->decrypt($request->id))
            ->where('campus', session('campus'))
            ->first();

        $ids = $patient->AgencyNumber;
    }

    $check = HealthHistory::where('patientId', $ids)->where('campus', session('campus'))->first();
    ...
    public function generateRSPDF(Request $request)
    {
     ...
     return Pdf::loadView('pages.referral-slip-form', compact('response', 'addYear', 'doctor'))->setPaper('legal')->stream();

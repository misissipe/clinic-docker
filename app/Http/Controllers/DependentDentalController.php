...
class DependentDentalController extends Controller
{
    protected $aes;
    ...
    public function addDependent(Request $request){
        ...
        $existingRecord = DB::table('dependent_info') ->select('id')->orderByRaw("CAST(SUBSTRING_INDEX(id, '-', -1) AS UNSIGNED) DESC")->where('campus',session('campus'))->first();
      
        if ($existingRecord) {
            $counter = intval(substr($existingRecord->id, strlen('D-')));
            $counter++; 
            $newId = 'D-' . $counter;
        } else {
            $newId = 'D-1';

 ...
 public function index(){
    ...
    public function indexRecord(Request $request){
      ...
      if($role === 'Student'){
        ...
        $name = Employee::where('AgencyNumber', (new AESCipher)->decrypt($request->id))->where('campus',session('campus'))->first();
        $data = DB::table('medicalrecord as m')
          ->select('m.*','e.LastName','e.FirstName','e.MiddleName','e.Sex','e.AgencyNumber','e.DateOfBirth')
          ->join('employee_info as e','m.patientId','=','e.AgencyNumber')
          ->where('e.AgencyNumber',(new AESCipher)->decrypt($request->id))
     ...
     return view('pages.patient-monitoring-record',['pageConfigs'=>$pageConfigs,'breadcrumbs'=>$breadcrumbs], compact('addYear','view', 'name', 'request','data','role','year','age','today','id'));
    ...
    public function autocomplete(Request $request){
      ...
      if ($role === 'Student') {
        ...
        $result = Employee::where('campus', session('campus'))
          ...
          $employee->encryptedEmployeeNo = (new AESCipher)->encrypt($employee->AgencyNumber);
          return $employee;
        });
      ...
      try {
        ...
        if ($existingRecord) {
          ...
          if (in_array("OTC Medicine", $request->purpose)) {
            ...
            $items = Stocks::whereIn('item_name', $OTCmedDescript)
                ->where('campus', session('campus'))
                ->where('item_quantity', '>', 0)
                ->get()
                ->keyBy('item_name'); 

            $latestInventories = Inventory::whereIn('stockId', $stockId)
                ->where('campus', session('campus'))
            ...
            foreach ($OTCmedDescript as $index => $description) {
                ...
                if (!isset($items[$description])) continue; 

                $stockItem = $items[$description];
            ...
            if($record === true) {
              return response()->json(['status' => 200,'success'   => 'Saved Successfully!','newId' =>   $new,'recId' =>   $ne->id,'role' => $role,'purpose' => $purpose  ]);
            } else {

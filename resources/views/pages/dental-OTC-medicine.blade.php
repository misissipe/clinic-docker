    ...
    <div class="col-12">
      <div class="card active">
          <div class="card-content">
              <div class="card-body">
                <label style="font-size:18px">OTC Medicine Record for the month of</label>
                <br><br>
                ...
                <div class="tab-content">
                  <div class="tab-pane active" id="home" aria-labelledby="home-tab" role="tabpanel">
                    <div class="table-responsive view-all active" id="todayDisapprove">
                      ...
                      <table class="table table-sm  cell-border " id="disToday">
                        <thead>
                            <tr>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:10%">date</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:10%">id number</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:15%">name of patient</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:5%">age</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:5%">sex</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:7%">course</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:15%">ASSESSMENT AND DIAGNOSIS/ES</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:15%">PLANNING AND INTERVENTION/S </th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:15%">MEDICINE/S  </th>
                                {{-- <th style="color:white;text-align: center">action</th> --}}
                            </tr>
                        </thead>
                        <tbody id="viewAllRecord">
                            @foreach ($otcdentalS as $data)
                            <tr>
                                <td style="text-align: center">{{ date('m-d-Y', strtotime($data->date)) }}</td>
                                <td style="text-align: center">{{ $data->patientId }}</td>
                                <td>{{utf8_decode( $data->lastname) }}, {{ utf8_decode($data->firstname) }} {{ utf8_decode($data->middlename) }}</td>                                <td style="text-align: center">{{ $data->age  }}</td>
                                <td style="text-align: center">{{ $data->gender }}</td>
                                <td>{{ $data->poscourse }}</td>
                                <td>{{ $data->diagnosis }}</td>
                                <td>{{ $data->treatment }}</td>
                                @php
                                    $pcs = json_decode($data->OTCmedpcs ?? '[]', true) ?: [];
                                    $desc = json_decode($data->OTCmedDescript ?? '[]', true) ?: [];
                                    $items = [];

                                    if (is_array($pcs) && is_array($desc)) {
                                        foreach ($pcs as $i => $qty) {
                                            if (!empty($qty) && !empty($desc[$i])) {
                                                $items[] = $qty . 'pcs - ' . ucwords(strtolower($desc[$i]));
                                            }
                                        }
                                    }
                                @endphp
                    ...
                    <table class="table table-sm  cell-border " id="empToday">
                      <thead>
                          <tr>
                              <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:10%">date</th>
                              <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:7%">id number</th>
                              <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:15%">name of patient</th>
                              <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:5%">age</th>
                              <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:5%">sex</th>
                              <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:15%">position</th>
                              <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:15%">ASSESSMENT AND DIAGNOSIS/ES</th>
                              <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:15%">PLANNING AND INTERVENTION/S </th>
                              <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:15%">MEDICINE/S  </th>
                              {{-- <th style="color:white;text-align: center">action</th> --}}
                  ...
                  <div class="tab-pane" id="profile" aria-labelledby="profile-tab" role="tabpanel">
                    <div class="table-responsive view-all">
                      <div class="btnprint">
                        <form action="/generate-OTC-medicine" method="get" target="_blank" class="float-right">
                        ...
                        <input type="hidden"name="firstname" id="roleSelect" value="Employee">
                          <thead>
                              <tr>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:10%">date</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:10%">id number</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:30%">name of patient</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:5%">age</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:5%">sex</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:7%">position/course</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:20%">ASSESSMENT AND DIAGNOSIS/ES</th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:20%">PLANNING AND INTERVENTION/S </th>
                                <th style="color:white;text-align: center;border: 1px solid rgb(226, 222, 222);width:20%">MEDICINE/S  </th>
                              </tr>
                          </thead>
                          <tbody >  
                    ...
                    <div class="table-responsive view-all">
                      <div id="printPart">
                        ...
                        <table class="table table-sm recordTable cell-border" id="print">
                          ...
                          <thead>
                              <tr>
                                  <th style="color:rgb(0, 0, 0); text-align: center; border: 1px solid rgb(0, 0, 0);width:10%">date</th>
                                  <th style="color:rgb(0, 0, 0); text-align: center; border: 1px solid rgb(0, 0, 0);width:7%">id number</th>
                                  <th style="color:rgb(0, 0, 0); text-align: center; border: 1px solid rgb(0, 0, 0);width:30%">Name of Patient</th>
                                  <th style="color:rgb(0, 0, 0); text-align: center; border: 1px solid rgb(0, 0, 0);width:5%">age</th>
                                  <th style="color:rgb(0, 0, 0); text-align: center; border: 1px solid rgb(0, 0, 0);width:5%">sex</th>
                                  <th style="color:rgb(0, 0, 0); text-align: center; border: 1px solid rgb(0, 0, 0);width:15%">position/course</th>
                                  <th style="color:rgb(0, 0, 0); text-align: center; border: 1px solid rgb(0, 0, 0);width:20%">ASSESSMENT AND DIAGNOSIS/ES</th>
                                  <th style="color:rgb(0, 0, 0); text-align: center; border: 1px solid rgb(0, 0, 0);width:20%">PLANNING AND INTERVENTION/S </th>
                                  <th style="color:rgb(0, 0, 0); text-align: center; border: 1px solid rgb(0, 0, 0);width:20%">MEDICINE/S  </th>
                              </tr>
                          </thead>
                          <tbody>
            ...
            success: function(data) {
                ...
                if (data.length > 0) {
                    $.each(data, function(index, record) {
                        ...
                        var fullname = (record.lastname).replace(/Ã±/g, 'ñ') + ', ' + 
                                        (record.firstname).replace(/Ã±/g, 'ñ') + 
                                        (record.middlename ? ' ' + record.middlename.replace(/Ã±/g, 'ñ') : '');
                        var otcMedDesc = record.OTCmedDescript ? JSON.parse(record.OTCmedDescript).join(', ') : '';
...
$(document).ready(function() {
    $('#submitToGenerate').click(function(event) {
        ...
        window.location.href = `/generate-OTC-medicine?monthSearch=${monthSearch}&year=${year}&role=${role}`;
    });
});

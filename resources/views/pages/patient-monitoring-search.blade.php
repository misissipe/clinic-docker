...
{{-- title --}}
@section('title','Patient Monitoring Record')
...
<script>
              ...
              let middleName = student.MiddleName ? student.MiddleName.replace(/ï¿½\?Â±|Ã±/g, 'ñ') : '';
                table.row.add([
                    ...
                    `<td style="text-align:center">
                      <a class="dropdown-item add" data-role="Student" data-campus="{{ session('campus') }}" href="#" data-id="${student.encryptedStudentNo}"><i class="fa fa-folder mr-1"></i></a>
                      </td>`
                ]).draw(false);
            });
            ...
            data.data.forEach(function(employee) {
              ...
              let middleName = employee.MiddleName ? employee.MiddleName.replace(/ï¿½\?Â±|Ã±/g, 'ñ') : '';
                table.row.add([
                    ...
                    `<td>${middleName}</td>`,
                    `<td style="text-align:center">
                      <a class="dropdown-item add" data-role="Employee" data-campus="{{ session('campus') }}" href="#" data-id="${employee.encryptedEmployeeNo}"><i class="fa fa-folder mr-1"></i></a>
                      </td>`
                ]).draw(false);
  ...
  if (role === 'Student') {
       console.log(id);
       if ( campus === 1){
         window.location.href = "/patient-monitoring-record-SG?id=" + encodeURIComponent(id) + "&role=Student";
       }else{
         window.location.href = "/patient-monitoring-record?id=" + encodeURIComponent(id) + "&role=Student";
       }
      
  } else if (role === 'Employee') {
    if ( campus === 1){
          window.location.href = "/patient-monitoring-record-SG?id=" + encodeURIComponent(id) + "&role=Employee";
       }else{
         window.location.href = "/patient-monitoring-record?id=" + encodeURIComponent(id) + "&role=Employee";
       }

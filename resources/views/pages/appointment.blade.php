  ...
  <div class="row d-flex justify-content-center">
    <div class="col-md-8" id="show">
      <div class="card-header" style="background-color:rgb(110, 155, 222);color:#ffffff;height:55px;font-size:15px;font-weight:800;display:flex;align-items:center;">
        ...
        <input type="text" class="cert col-sm-10 name" name="firstname" id="name"value="{{ $record->FirstName ?? $course->FirstName }} {{ $record->MiddleName ?? $course->MiddleName }} {{ $record->LastName  ?? $course->LastName }}"   style="background-color:transparent;color:#ffffff;font-size:19px;font-weight:500" readonly>
      </div>          
      <div class="card text-left">
        <div class="card-body">
          <form action="/setAppointment"  method="post" id="appointmentForm">
            @csrf  
              <input class="form-control" type="hidden" name="role" id="role"  value="{{$role}}">
              <input class="form-control" type="hidden" name="patientId" id="id"  value="{{$record->AgencyNumber ?? $course->StudentNo}}">
              <input class="form-control" type="hidden" name="status" id="Pending" value="Pending">
              <input class="form-control lastname" type="hidden" name="lastname" id="lastname" value="{{$record->LastName ?? $course->LastName}} ">
              <input class="form-control firstname" type="hidden" name="firstname" id="firstname" value="{{$record->FirstName ?? $course->FirstName}} ">
              <input class="form-control middlename" type="hidden" name="middlename" id="middlename" value="{{$record->MiddleName ?? $course->MiddleName}} ">
              <div style="font-weight: 400; font-size: 20px;">
                <div class=" col-md-12">
                  <label for="purpose" style="display: inline-block;width:15%">Contact Number: </label>
                  <input type="text" id="contactNo" style="display: inline-block;" class="form-control col-sm-5 @error('contactNo') is-invalid @enderror" name="contactNo" value="{{$record->Cellphone ?? $course->ContactNo}} " required autocomplete="contactNo" />
              </div>
              <br>
              <div class=" col-md-12">
      ...
      if (response.status == 200) {
        Swal.fire({
          ...
          if (response1.isConfirmed) {
           window.location.href = "/appointment-search"
          }
        });

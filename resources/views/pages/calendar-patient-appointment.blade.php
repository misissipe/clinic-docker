...
<section id="basic-datatable">
                    ...
                    <div id="step2" style="display: none;">
                        <div class="form-group">
                            <form action="/setReservation" method="post" id="appointmentForm">
                                ...
                                <input class="form-control" type="hidden" name="status" id="Pending" value="Reserved">
                                ...
                                <div style="font-weight: 400; font-size: 20px;">
                                    <div class="col-md-12">
                                        <label for="purpose" style="display: inline-block;width:20%">Contact Number: </label>
    ...
    <div class="modal fade" id="DetailsModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        </div>a@extends('layouts.contentLayoutMaster')
{{-- title --}}
@section('title','Calendar')
...
$(document).ready(function() {
    ...
    // Cancel past reservations if status is 'Reserved'
    ...
    if (moment(event.date).isBefore(moment(), 'day')) {
        if (event.status === 'Reserved' || event.status === 'Pending') {
            $.ajax({
                ...
                data: { id: event.id, date: event.date, time: event.time, },
                success: function(response) {
                    if (response.status === 200) {
                        console.log(`Past reservation on ${event.date} with status '${event.status}' automatically canceled.`);
                    } else {
                        console.error(`Failed to cancel reservation on ${event.date}: ${response.error}`);
                    }
                },
                error: function() {
                    console.error(`Error occurred while canceling reservation on ${event.date}.`);
                },
            });
            ...
            $.ajax({
                type: 'POST',
                url: '/updateStatus',
                data: { id: event.id, status: 'Done' },
                success: function(response) {
                    if (response.status === 200) {
                        console.log(`Past reservation on ${event.date} marked as 'Done'.`);
                    } else {
                        console.error(`Failed to update status to 'Done' on ${event.date}: ${response.error}`);
                    }
                },
                error: function() {
                    console.error(`Error occurred while updating status to 'Done' on ${event.date}.`);
                },
            });
            ...
            $.ajax({
                type: 'POST',
                url: '/updateStatus',
                data: { id: event.id, status: 'Reschedule' },
                success: function(response) {
                    if (response.status === 200) {
                        console.log(`Past reservation on ${event.date} marked as 'Reschedule'.`);
                    } else {
                        console.error(`Failed to update status to 'Reschedule' on ${event.date}: ${response.error}`);
                    }
                },
                error: function() {
                    console.error(`Error occurred while updating status to 'Reschedule' on ${event.date}.`);
                },
            });
        }
    }
...
// Initialize FullCalendar with settings
...
$('#calendar').on('click', '.view-details', function() {
    ...
    const eventDetails = reserve.find((event) => event.id === eventId);
...
$('#availableSlots').on('click', '.available-slot', function() {

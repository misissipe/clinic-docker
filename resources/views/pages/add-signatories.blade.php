...
<section id="basic-datatable">
  <div class="row justify-content-center">
    <div class="col-12">
      <div class="card">
        ...
        <div class="card-body">
          <button type="button" class="btn btn-primary float-right button" data-toggle="modal" data-target="#addSignatories" id="addsignatories">
            <a style="color: rgb(255, 255, 255); font-size: 15px;">Add Signatories</a>
          </button>
            ...
            <table class="table table-sm recordTable table-bordered table-striped zero-configuration" id="recordTable" style="text-align:center;">
              ...
              @foreach($view as $data)  
                <tr>    
                  ...
                  <td style="text-transform:capitalize;">{{$data->Designation}}</td>
                  <td style="text-transform:capitalize;">{{$data->Office}}</td>
                  <td style="text-transform:capitalize;">{{$data->EmploymentStatus}}</td>
  ...
  @include('modal.addSignatories')
  @include('modal.editSignatories')
</section>
@endsection
{{-- vendor scripts --}}
...
$("#addSignatoriesModal").submit(function(e) {
e.preventDefault();
  ...
  $(document).on('click', '.editModal', function() {
    ...
    $.ajax({
        ...
        success: function(response) {
          console.log(response);
          $('#editSignatories').modal('show');
          console.log(response.EmploymentStatus);
          ...
          $('.emp').val(response.EmploymentStatus);
        } 
    });
  });
});
...
$(document).ready(function() {
  $('#editSignatoriesModal').submit(function(e) {
    e.preventDefault();
      ...
      success: function(response) {
        if (response.status == 200) {
          ...
          confirmButtonText: 'Okay',
            ...
            if (response.isConfirmed) {
              $('#editSignatoriesModal').modal('hide')
                console.log(response); 
                location.reload();			
            }
          })

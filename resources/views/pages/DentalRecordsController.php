...
#Patient Treatment Record
    ...
    public function treatmentList(Request $request)
    {
        ...
        if (empty($existingTreatment)) {
            ...
            $check =  DB::table('dentalchart')->where('id',$this->aes->decrypt($request->id))->where('campus',session('campus'))->first();
        ...
        $name = DB::table('dentalchart')->where('id', $this->aes->decrypt($request->id))->where('campus',session('campus'))->first();
        ...
        $view = DB::table('treatmentrecord as t')
                ->select('t.*','s.lastname','s.firstname','s.middlename','s.gender')
                ->join('dentalchart as s', function ($join) {
                    $join->on('t.patientId', '=', 's.id')
...
// For denatal Assessment Questionnaire
 public function indexAssessmentStudent()
     ...
     public function indexQuestionnaire(Request $request)
    {
        ...
        return view('pages.assessment-questionnaire',compact('data','age','gender'),['pageConfigs'=>$pageConfigs,'breadcrumbs'=>$breadcrumbs],compact('appointments'));
    }
}
